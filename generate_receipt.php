...
// 1. Fetch Admin Details (The person currently logged in)
$adminStmt = $pdo->prepare("SELECT name, position FROM users WHERE id = ?");
$adminStmt->execute([$_SESSION['user_id']]);
$admin = $adminStmt->fetch(PDO::FETCH_ASSOC);
$adminName = !empty($admin['name']) ? $admin['name'] : 'Admin';
$adminPosition = !empty($admin['position']) ? $admin['position'] : 'IT Administrator';

// 2. Fetch Order, Staff Details, and Live Stock Balance (c.quantity)
$reqStmt = $pdo->prepare("
    SELECT r.quantity_requested, r.request_date, r.status,
           u.name as staff_name, u.position as staff_position, u.department,
           c.colour, c.quantity as current_balance, m.brand, m.model_name
    FROM requests r
    JOIN users u ON r.user_id = u.id
    JOIN ink_colours c ON r.colour_id = c.colour_id
    JOIN ink_models m ON c.model_id = m.model_id
    WHERE (r.batch_id = ? OR r.id = ?)
");
...
<head>
    ...
    <title>Resit Rasmi - <?= htmlspecialchars($batch_id) ?></title>
    ...
    <style>
        ...
        .document-container { max-width: 900px; margin: 40px auto; background: #fff; padding: 50px; border: 1px solid #ccc; box-shadow: 0 5px 15px rgba(0,0,0,0.1); }
        ...
        .item-table { border: 2px solid #000; min-width: 600px; }
        .item-table th, .item-table td { border: 1px solid #000; padding: 10px; color: #000; }
        .item-table th { background-color: #f8f9fa; font-weight: bold; text-transform: uppercase; font-size: 0.8rem; }
        
        .signature-section { margin-top: 50px; page-break-inside: avoid; }
        .sig-box { border: 1px solid #000; padding: 15px; height: 160px; position: relative; }
        .sig-line { border-bottom: 1px solid #000; width: 100%; margin-top: 40px; margin-bottom: 5px; }
        .sig-label { font-size: 0.8rem; font-weight: bold; text-transform: uppercase; }
...
<body>
    ...
    <div class="document-container">
        ...
        <div class="d-flex justify-content-end mb-4 no-print gap-2">
            ...
            <button onclick="window.print()" class="btn btn-dark fw-bold"><i class="fa-solid fa-print me-1"></i> Cetak Dokumen</button>
        </div>
        ...
        <table class="info-table">
            <tr>
                ...
                <td style="width: 50%;">
                    ...
                    <p class="mb-1 small"><strong>Tarikh Permohonan (Sistem):</strong> <?= $requestDate ?></p>
            ...
            <table class="table item-table mb-4">
                <thead>
                    <tr>
                        ...
                        <th style="width: 45%;">Penerangan Barangan Inventori</th>
                        <th style="width: 20%; text-align: center;">Warna</th>
                        <th style="width: 15%; text-align: center;">Baki Sedia Ada</th>
                        <th style="width: 15%; text-align: center;">Kuantiti Diluluskan</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        <td colspan="4" class="text-end fw-bold text-uppercase bg-light">Jumlah Kuantiti Diluluskan</td>
        ...
        <!-- Formal Signatures -->
        <!-- Ditukar kepada col-12 col-md-6 untuk reponsif -->
        <div class="row signature-section g-0">
            <div class="col-12 col-md-6 pe-md-2 mb-3 mb-md-0">
                <div class="sig-box">
                    <span class="sig-label">Diluluskan oleh</span>
                    <div class="sig-line"></div>
                    <p class="mb-0 small"><strong>Nama:</strong> <?= htmlspecialchars($adminName) ?></p>
                    <p class="mb-0 small"><strong>Tarikh:</strong> ______________________</p>
                </div>
            </div>
            <div class="col-12 col-md-6 ps-md-2">
                <div class="sig-box">
                    ...
                    <div class="sig-line"></div>
                    <p class="mb-0 small"><strong>Nama:</strong> <?= htmlspecialchars($staffData['staff_name']) ?></p>
                    <p class="mb-0 small"><strong>Tarikh:</strong> ______________________</p>
                </div>
            </div>
        </div>
